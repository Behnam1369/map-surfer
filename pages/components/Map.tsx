...
export default function Map() {
  ...
  const [mapPosition, setMapPosition] = useState<coordinate>({
    x: -128,
    y: -128,
  });
  const [showPointer, setShowPointer] = useState<boolean>(false);
  const [pointerPosition, setPointerPosition] = useState<coordinate>({
    x: -100,
    y: -100,
  });
  ...
  return (
    <div className={style.container}>
      ...
      >
        <img
          src={`${tileServer}/${zoom}/${coordinate.x - 1}/${
            coordinate.y - 1
          }.png`}
        />
        <img
          src={`${tileServer}/${zoom}/${coordinate.x}/${coordinate.y - 1}.png`}
        />
        <img
          src={`${tileServer}/${zoom}/${coordinate.x + 1}/${
            coordinate.y - 1
        ...
        />
        <img
          src={`${tileServer}/${zoom}/${coordinate.x - 1}/${coordinate.y}.png`}
        />
        <img
          src={`${tileServer}/${zoom}/${coordinate.x}/${coordinate.y}.png`}
        />
        <img
          src={`${tileServer}/${zoom}/${coordinate.x + 1}/${coordinate.y}.png`}
        />
      ...
      {showPointer && (
        <Pointer
          ...
          location={coordinate}
          onChange={(val: string) => setLocationTitle(val)}
        />
      )}
      {locationTitle && (
        <p className={style.location} dir="ltr">
          Location: {locationTitle}{" "}
        </p>
      )}
      <Zoom handleZoom={(val: number) => handleZoom(val)} zoomValue={zoom} />
      <Search
        onSelect={(street: { zoom: number; x: number; y: number }) =>
          handleSelect(street)
        }
      />
    </div>
